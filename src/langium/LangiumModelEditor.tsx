...
export function LangiumModelEditor(props: LangiumModelEditorProps) {
  ...
  useEffect(() => {
    let mounted = true;
    async function init() {
      worker.current = await createWorker(props.grammar, props.value.ns);
      ...
      if (mounted) {
        const ast = modelSerializer.deserialize(props.value, { grammar: Grammar });
...
function createWorker(grammar: string, namespaces: Record<string, string>): Promise<Worker> {
  return new Promise((resolve, reject) => {
    ...
    worker.postMessage({ type: 'start', grammar, namespaces });
  });
}
